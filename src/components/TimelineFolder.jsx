...
const TimelineFolder = ({ children, year, title, images = [] }) => {
  ...
  const fanStyles = [
    { rotate: -15, x: -80, y: -70, scale: 0.95 },
    { rotate: 5, x: 80, y: -80, scale: 1.05 },
    { rotate: -5, x: 0, y: -120, scale: 1.0 }
  ];

  return (
    <div className="p-6 rounded-2xl border-2 border-black dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-md hover:shadow-lg transition-all flex flex-col justify-between relative group">
      {/* Outer Card Content */}
      <div className="flex-1">
        {children}
      </div>

      {/* Folder Trigger Button Section */}
                ...
                {images.map((imgUrl, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="absolute w-32 h-20 sm:w-40 sm:h-28 rounded-lg overflow-hidden border-2 border-black dark:border-white shadow-2xl bg-zinc-100 dark:bg-zinc-800 z-20 cursor-zoom-in"
